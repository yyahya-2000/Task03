...
void decoding(int i, const string& encryptedMessage) {
	results = new string[i]; // becuase of we know the num of threads so we initialize the dynamic array results 
	make_tasks(i, encryptedMessage); // split tasks in order to give task to each thread
	thrs = new thread[i];
	int index = 0;
	while (true)
	{
		...
		thrs[index] = thread(encrypte_message, curtask, index); //solve new task
